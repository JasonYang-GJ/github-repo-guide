...
const COPY = {
  "zh-CN": {
    ...
    privacyBody: "连接配置保存在当前浏览器；你填写的密钥只留在页面内存，不写入报告或日志，刷新后需重新填写。",
    ...
    reportModeLegend: "报告方式", analysisModeLabel: "分析模式", deterministicOption: "免费分析（不调用模型）",
    ...
    credentialLifecycle: "密钥使用说明", browserMemory: "密钥仅留在页面内存", oneLocalRequest: "不写入报告或日志", discardAfterRequest: "刷新页面后需重新填写",
    ...
    guideBasis: "文档与仓库信息 · 不代表已验证运行效果", guideInterpretation: "AI 详细解读 · 需结合原文核对", guideFeatures: "主要功能与特点", guideUsage: "怎样开始使用", guideCaveats: "使用前注意", guideDocumented: "仓库文档声明", guideInferred: "AI 辅助判断 · 尚未验证", guideMissing: "所选文档未提供明确说明，请进一步查看仓库。",
    ...
    filesSeen: "已发现文件", filesRead: "实际读取", evidenceRecords: "证据记录", coreModules: "核心模块", analysisDuration: "分析耗时", analysisModeMetric: "分析模式", modelUsage: "模型用量", repositoryExecution: "仓库执行", criticalGates: "检查记录",
  ...
  en: {
    ...
    privacyBody: "Connection settings stay in this browser. Entered keys stay in page memory, not reports or logs. Re-enter them after reloading.",
    ...
    reportModeLegend: "Report options", analysisModeLabel: "Analysis mode", deterministicOption: "Free analysis (no model)",
    ...
    credentialLifecycle: "Key handling", browserMemory: "Keys stay in page memory", oneLocalRequest: "Never in reports or logs", discardAfterRequest: "Re-enter after reloading",
    ...
    filesSeen: "Files seen", filesRead: "Files read", evidenceRecords: "Evidence records", coreModules: "Core modules", analysisDuration: "Duration", analysisModeMetric: "Analysis mode", modelUsage: "Model usage", repositoryExecution: "Repository execution", criticalGates: "Check results",
...
const RUNTIME_COPY = {
  "zh-CN": {
    ...
    modeDeterministic: "免费模式按固定规则整理报告，不调用模型，也不运行仓库代码。",
    ...
    resultKicker: "项目分析报告", fallbackResultKicker: "项目分析报告 · 免费模式", deterministicFree: "免费分析", paidModel: "{model}（付费）", tokenUsage: "{input} 入 / {output} 出", noPaidTokens: "0 付费 Token",
    ...
    reportLanguageHeading: "当前报告是{reportLanguage}", reportLanguageMessage: "界面已切换，报告仍保留原语言。如需更换报告语言，请重新分析；切换语言本身不会调用模型。", chinese: "中文", english: "英文",
  ...
  en: {
    ...
    modeDeterministic: "Free mode uses fixed rules to organize the report. No model calls or repository code execution.",
    ...
    resultKicker: "Repository report", fallbackResultKicker: "Repository report · Free mode", deterministicFree: "Free analysis", paidModel: "{model} (paid)", tokenUsage: "{input} in / {output} out", noPaidTokens: "0 paid tokens",
    ...
    reportLanguageHeading: "This report was generated in {reportLanguage}", reportLanguageMessage: "The interface has changed; the report keeps its original language. Run the analysis again for a report in this language. Switching languages does not call a model.", chinese: "Chinese", english: "English",
...
const reportLanguageNotice = document.querySelector("#report-language-notice");
...
function reportLanguageName(locale) {
...
function providerChanged(reset = false) {
  ...
  modeSummary.textContent = usesDeepSeek
    ...
    : t("modeDeterministic");
  setText("#ai-mode-option", modelText("AI 分析（自选供应商）", "AI analysis (your provider)"));
...
function renderGuide(data) {
  ...
  document.querySelector("#guide-interpretation").hidden = !model;
...
function renderResult(data, scroll = true) {
  ...
  const reportLocale = data.locale === "en" ? "en" : "zh-CN";
  const languageMismatch = reportLocale !== currentLocale;
  reportLanguageNotice.hidden = !languageMismatch;
  if (languageMismatch) {
    setText(
      "#report-language-heading",
      t("reportLanguageHeading", { reportLanguage: reportLanguageName(reportLocale) }),
    );
    setText("#report-language-message", t("reportLanguageMessage"));
  }
  ...
  if (scroll) resultShell.scrollIntoView({ behavior: "smooth", block: "start" });
}
...
form.addEventListener("submit", async (event) => {
  ...
  if (provider === "custom" && (!selectedConnection || !selectedConnection.apiKey.trim())) {
    showError("MODEL_API_KEY_REQUIRED", modelText("请先添加供应商、选择模型，并填写这个供应商自己的 API Key。", "Add a provider, select a model and enter its API key first."));
  ...
  try {
    ...
    if (provider !== "deterministic") {
      request.apiKey = selectedConnection.apiKey.trim();
